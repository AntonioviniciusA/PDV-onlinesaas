...
interface EtiquetasPrecoProps {
  onBack: () => void
  products: Product[]
}

export default function EtiquetasPreco({ onBack, products }: EtiquetasPrecoProps) {
  ...
  const handleBarcodeSubmit = (e: React.FormEvent) => {
    ...
    const existingItem = printList.find((item) => item.product.id === product.id)
  ...
  const updateQuantity = (productId: string, quantity: number) => {
    if (quantity <= 0) {
      ...
      setPrintList((prev) => prev.map((item) => (item.product.id === productId ? { ...item, quantity } : item)))
    }
  }
      ...
      <div className="flex-shrink-0 p-4 border-b bg-white">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Etiquetas de Preço</h1>
              <p className="text-sm text-gray-600">Gerar etiquetas para produtos</p>
            </div>
          </div>
              ...
              <CardContent className="flex-1 flex flex-col">
                <form onSubmit={handleBarcodeSubmit} className="space-y-4">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Escaneie o código de barras ou digite manualmente e pressione Enter
                    </p>
                  </div>
                ...
                <div className="bg-blue-50 p-4 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-blue-900">{printList.length}</p>
                    </div>
                    <div>
                      <p className="text-blue-600">Total de Etiquetas</p>
                      <p className="text-2xl font-bold text-blue-900">{totalLabels}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                  ...
                  <ol className="text-sm text-gray-600 space-y-1">
                    ...
                    <li>2. O produto será adicionado à lista automaticamente</li>
                ...
                {printList.length === 0 ? (
                  <div className="flex items-center justify-center h-full">
                    <div className="text-center">
                      ...
                      <p className="text-sm text-gray-400 mt-2">Escaneie códigos de barras para adicionar produtos</p>
                    </div>
                  </div>
                ) : (
                  <ScrollArea className="h-full">
                    <div className="space-y-3">
                      {printList.map((item) => {
                        const hasComparison =
                          item.product.quantity && item.product.measureUnit && item.product.quantity > 0
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  {comparison && (
                                    <Calculator className="w-3 h-3 text-blue-600" title="Com comparação de preço" />
                                  )}
                                ...
                                <div className="text-xs text-gray-500 space-y-1">
                                  ...
                                  <p className="font-semibold text-green-600">
                                    ...
                                    {item.product.requiresWeight ? "/kg" : `/${item.product.unit}`}
                                  </p>
                                  {comparison && comparison.comparison && (
                                    <p className="font-semibold text-blue-600">{comparison.comparison}</p>
                                  )}
                                  {hasComparison && (
                                    <p className="text-gray-400">
                                      {item.product.quantity} {item.product.measureUnit}
                                    </p>
                                  )}
                                </div>
                              </div>
            ...
            <Card className="mt-4">
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {printList.map((item) => {
                    const hasComparison = item.product.quantity && item.product.measureUnit && item.product.quantity > 0
                    const comparison = hasComparison
                      ? calculateComparativePrice(item.product.price, item.product.quantity!, item.product.measureUnit!)
                      : null
                    ...
                    return Array.from({ length: item.quantity }).map((_, index) => (
                      ...
                      >
                        <div className="text-xs font-bold truncate mb-1">{item.product.name}</div>
                        <div className="text-lg font-bold text-green-600">R$ {item.product.price.toFixed(2)}</div>
                        <div className="text-xs text-gray-500">
                          {item.product.requiresWeight ? "/kg" : `/${item.product.unit}`}
                        </div>
                        {comparison && comparison.comparison && (
                          <div className="text-xs text-blue-600 font-semibold mt-1">{comparison.comparison}</div>
                        )}
                        {item.product.icms && (
                          <div className="text-xs text-gray-500 mt-1">ICMS {item.product.icms}%</div>
                        )}
                        <div className="text-xs font-mono mt-1">{item.product.barcode}</div>
                      </div>
                    ))
                  })}
                </div>
                <p className="text-sm text-gray-500 mt-4">
                  Preview das etiquetas que serão impressas. Tamanho real: 140mm × 100mm
                </p>
              </CardContent>
            </Card>
          )}
