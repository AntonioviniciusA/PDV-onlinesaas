...
export function CSVImportDialog({ open, onOpenChange, products, onImportComplete }: CSVImportDialogProps) {
...
Banana Prata,5.50,Frutas,2000000000002,sim,kg`
  ...
  const validateRow = (row: any, index: number): ImportRow => {
    ...
    const existingInFile = csvData.find(
      (item) => item.barcode === row.codigo_barras?.toString() && item.rowIndex !== index,
    )
    if (existingInFile) {
    ...
    const existingProduct = products.find((p) => p.barcode === row.codigo_barras?.toString())
          ...
          {step === "upload" && (
            <div className="space-y-4">
              ...
              >
                ...
                <h3 className="text-lg font-semibold mb-2">Arraste o arquivo CSV aqui ou clique para selecionar</h3>
                <p className="text-gray-500 mb-4">
                  Formato suportado: CSV com colunas: nome, preco, categoria, codigo_barras, por_peso, unidade
                </p>
                <input
                  ref={fileInputRef}
                  type="file"
              ...
              <Alert>
                ...
                <AlertDescription>
                  ...
                  <br />• Colunas: nome, preco, categoria, codigo_barras, por_peso (sim/nao), unidade
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <Badge variant="secondary">{csvData.length} linhas encontradas</Badge>
                  <Badge variant={validRows > 0 ? "default" : "secondary"}>{validRows} válidas</Badge>
                  {errorRows > 0 && <Badge variant="destructive">{errorRows} com erro</Badge>}
                </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {csvData.map((row, index) => (
                        <tr key={index} className={`border-b ${!row.isValid ? "bg-red-50" : ""}`}>
                          ...
                          <td className="p-2">{row.requiresWeight === "sim" ? "Sim" : "Não"}</td>
                          <td className="p-2">
                            {row.isValid ? (
              ...
              <div className="flex gap-2 justify-end">
                ...
                <Button onClick={processImport} disabled={validRows === 0 || isProcessing}>
                  {isProcessing ? (
                    <>
                  ...
                  <div className="p-4 space-y-2">
                    {importResult.details.map((detail, index) => (
                      <div key={index} className="flex items-center gap-3 p-2 border rounded">
                        {detail.status === "success" && <CheckCircle className="w-4 h-4 text-green-600" />}
                        {detail.status === "error" && <XCircle className="w-4 h-4 text-red-600" />}
                        {detail.status === "duplicate" && <AlertTriangle className="w-4 h-4 text-yellow-600" />}
