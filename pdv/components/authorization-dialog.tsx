...
interface AuthorizationDialogProps {
  ...
  title: string
  description: string
  onAuthorize: (supervisor: string) => void
}

// Códigos de autorização válidos (simulando cartões de cancelamento)
const VALID_AUTHORIZATION_CODES = {
  AUTH001FISCAL2024: { name: "João Silva", level: "fiscal", expires: "2024-12-31" },
  AUTH002SUPER2024: { name: "Maria Santos", level: "supervisor", expires: "2024-12-31" },
  AUTH003GERNT2024: { name: "Carlos Oliveira", level: "gerente", expires: "2024-12-31" },
  CANCEL123456789: { name: "Fiscal Geral", level: "fiscal", expires: "2024-12-31" },
  SUPER987654321: { name: "Supervisor Geral", level: "supervisor", expires: "2024-12-31" },
}

function decryptAuthCode(code: string) {
  // Simula descriptografia do código
  // Em um sistema real, isso seria uma descriptografia real
  const cleanCode = code.trim().toUpperCase()
  return VALID_AUTHORIZATION_CODES[cleanCode as keyof typeof VALID_AUTHORIZATION_CODES] || null
}

export function AuthorizationDialog({ open, onOpenChange, title, description, onAuthorize }: AuthorizationDialogProps) {
        ...
        <div className="space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              />
              <p className="text-xs text-gray-500 mt-1">Passe o cartão de cancelamento no leitor ou digite o código</p>
            </div>
