...
interface MixedPaymentDialogProps {
  ...
  onRequestAuthorization?: (request: any) => void
}
...
export function MixedPaymentDialog({
  ...
  const [pixPaymentStatus, setPixPaymentStatus] = useState<"pending" | "paid" | "denied" | "expired">("pending")
  ...
  const simulatePixPayment = (status: "paid" | "denied") => {
  ...
  const requestPixAuthorization = () => {
    if (onRequestAuthorization) {
      onRequestAuthorization({
        ...
        description: "Autorizar finalização da venda sem confirmação do PIX. Esta ação requer autorização do fiscal.",
  ...
  const addPayment = () => {
    ...
    const paymentName = paymentOptions.find((p) => p.id === newPaymentType)?.name || newPaymentType
        ...
        <div className="space-y-4">
          ...
          <div className="bg-blue-50 p-4 rounded-lg">
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                ...
                <p className="font-bold text-blue-600">R$ {totalAmount.toFixed(2)}</p>
              </div>
              <div>
            ...
            <div className="space-y-2">
              ...
              {payments.map((payment) => (
                <div key={payment.id} className="flex items-center justify-between p-3 border rounded-lg">
                  ...
                  <Button size="sm" variant="outline" onClick={() => removePayment(payment.id)}>
              ...
              <div className="flex flex-col items-center space-y-3 bg-gray-50 p-4 rounded-lg">
                <div className="w-32 h-32 bg-white border-2 border-gray-300 rounded-lg flex items-center justify-center relative">
                  {pixPaymentStatus === "pending" ? (
                    ...
                    <div className="text-center">
                      {pixPaymentStatus === "paid" && <CheckCircle className="w-8 h-8 text-green-600" />}
                      {pixPaymentStatus === "denied" && <XCircle className="w-8 h-8 text-red-600" />}
                      {pixPaymentStatus === "expired" && <Clock className="w-8 h-8 text-gray-600" />}
                ...
                <div className="text-center">
                  {pixPaymentStatus === "pending" && (
                    <p className="text-sm text-blue-600 font-semibold">Aguardando PIX... {formatTime(pixTimeLeft)}</p>
                  )}
                  {pixPaymentStatus === "paid" && (
                    <p className="text-sm text-green-600 font-semibold">✅ PIX CONFIRMADO</p>
                  )}
                  {pixPaymentStatus === "denied" && <p className="text-sm text-red-600 font-semibold">❌ PIX NEGADO</p>}
                  {pixPaymentStatus === "expired" && (
                    <p className="text-sm text-gray-600 font-semibold">⏰ PIX EXPIRADO</p>
                  )}
                  {pixAuthorizedByFiscal && (
                    <p className="text-sm text-orange-600 font-semibold">🛡️ AUTORIZADO PELO FISCAL</p>
                  )}
                </div>
                ...
                {pixPaymentStatus === "pending" && (
                  <div className="flex gap-2">
                    ...
                    >
                      {isSimulatingPix ? <RefreshCw className="w-3 h-3 animate-spin" /> : "Simular Pago"}
                    </Button>
                    <Button
                      size="sm"
                      variant="outline"
                    ...
                    >
                      {isSimulatingPix ? <RefreshCw className="w-3 h-3 animate-spin" /> : "Simular Negado"}
                    </Button>
                  </div>
                )}

                {(pixPaymentStatus === "denied" || pixPaymentStatus === "expired") && (
                  <div className="flex gap-2">
                    <Button size="sm" onClick={generateNewPixQR} variant="outline">
          ...
          {!isComplete && (
            <div className="space-y-3 border-t pt-4">
              ...
              <Button onClick={addPayment} disabled={!newPaymentType || !newPaymentAmount} className="w-full">
                Adicionar Pagamento
              </Button>
            </div>
          )}
          ...
          {remaining > 0.01 && (
            <Alert>
              ...
              <AlertDescription>Ainda faltam R$ {remaining.toFixed(2)} para completar o pagamento.</AlertDescription>
            </Alert>
          )}

          {hasPixPayment && pixPaymentStatus === "pending" && Math.abs(remaining) < 0.01 && (
            <Alert>
              <Clock className="h-4 w-4" />
              <AlertDescription>Aguardando confirmação do PIX para finalizar a venda.</AlertDescription>
            </Alert>
          )}
          ...
          {hasPixPayment && pixPaymentStatus === "denied" && (
            <Alert variant="destructive">
              ...
              <AlertDescription>PIX negado. Gere um novo QR Code ou solicite autorização do fiscal.</AlertDescription>
            </Alert>
          )}
          ...
          {hasPixPayment && pixPaymentStatus === "expired" && (
            <Alert>
              <Clock className="h-4 w-4" />
              <AlertDescription>
                QR Code PIX expirado. Gere um novo código ou solicite autorização do fiscal.
              </AlertDescription>
            </Alert>
          )}
          ...
          {isComplete && hasPixPayment && (pixPaymentStatus === "paid" || pixAuthorizedByFiscal) && (
            <Alert className="border-green-200 bg-green-50">
              <AlertDescription className="text-green-800">
                ✅ Pagamento completo e PIX confirmado! Pode finalizar a venda.
              </AlertDescription>
            </Alert>
          )}
