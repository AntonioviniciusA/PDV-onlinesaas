...
const PDVLoginDialog = ({ isOpen, onClose, onAuthSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await AuthService.loginPDV({
      ...
      if (response.token && response.codigo_acesso) {
        ...
        setLoginSuccess(
          "Login PDV realizado com sucesso! Seu código de acesso foi gerado."
        );
      ...
      setLoginError(
        error.response?.data?.message ||
          "Erro ao fazer login PDV. Verifique seus dados."
      );
      console.error("Erro no login PDV:", error);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex-1 p-6">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-black mb-2">
              {showAccessCode ? "Código de Acesso PDV" : "Acesso PDV"}
            </h1>
            <p className="text-gray-600">
              {showAccessCode
                ? "Use este código para conectar o PDV ao servidor"
                : "Faça login para obter seu código de acesso ao PDV"}
            </p>
          </div>
